...
# Version 1.0                                  #
# Date: 20.11.2015                             #
# Author: Ivo Klimsa i.klimsa@seznam.cz        #     
################################################

#For enabling debuging remove # from below line
#set -x
version=1.0.1
admins=("ivo.klimsa" "foo.whatever")
ident=$(hostname -s)
finfo="/etc/info"
...
config_mail () {
    ...
    echo "relayhost = smtp.whatever.com" > /etc/postfix/main.cf
...
create_monitoring_user () {
    #Adding user Icinga for monitoring
    echo "$(tput bold)$(tput setaf 2)Adding user Orion for monitoring $(tput sgr 0)"
    useradd -n  -g users -c "User for script based monitoring from Icinga" monitor
    echo whatever | passwd monitor --stdin
    usermod -g monitors monitor
...
create_admins () {
    ...
    for admin in "${admins[@]}" ;
        do
            pass=$(random-string)
            useradd -n  -g administrators -c "${admin}@whatever.com Sys Administrator" ${admin}
            echo ${pass} | passwd ${admin} --stdin
            chage -d 0 ${admin}
            echo -e "New credentials has been generated for you. \nLogin: ${admin} \nPassword: ${pass} \nServer: ${ident} " | mailx -r "noreplay@whatever.com" -s "Password for ${ident} " "${admin}@whatever.com"
        done
}
